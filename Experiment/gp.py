...
class GaussianProcess:
  ...
  def train(self):
    ...
    self.K = kk + self.params.s*np.eye(kk.shape[0])
  ...
  def add_sample(self, x, y):
    ...
    if self.params.gp_y.shape[1] > self.params.msn:
      for ii in range(0, self.params.gp_y.shape[1]-1):
        self.params.gp_x[:,ii] = self.params.gp_x[:,ii+1]
        self.params.gp_y[:,ii] = self.params.gp_y[:,ii+1]

      self.params.gp_x[:,-1] = x.reshape((x.shape[0],))
      self.params.gp_y[:,-1] = y
    else:
  ...
  def posterior_ddxmean(self, x):
    x = x.reshape(self.params.gp_x[:,0].shape)
    
    H = np.zeros((x.shape[0], x.shape[0], self.params.gp_x.shape[1]))
    ddx = np.zeros((x.shape[0], x.shape[0]))

    k = self.k(x, self.params.gp_x, self.params.tau)
    tau = self.params.tau

    for ii in range(0, x.shape[0]):
      for jj in range(0, x.shape[0]):
        for kk in range(0, self.params.gp_x.shape[1]):
          xkk = self.params.gp_x[:, kk]
          if ii == jj:
            H[ii, jj, kk] = -k[0][kk]/(tau[ii]**2) + (x[ii] - xkk[ii])**2*k[0][kk]/(tau[ii]**4)
          else:
            H[ii, jj, kk] = (x[ii] - xkk[ii])*(x[jj] - xkk[jj])*k[0][kk]/(tau[ii]*tau[jj])
    
    alpha = self.params.gp_alpha
    for kk in range(0, self.params.gp_x.shape[1]):
      ddx = ddx + H[:, :, kk]*alpha[kk]
    return ddx
  ...
  def posterior_ddxvariance(self, x):
    x = x.reshape(self.params.gp_x[:,0].shape)
    kk = self.k(x, self.params.gp_x, self.params.tau)
    kdx = self.kdx(x, self.params.gp_x, self.params.tau)

    dkx = np.zeros((kdx.shape[2], kdx.shape[1]))
    for i in range(kdx.shape[2]):
      dkx[i, :] = kdx[0, :, i]

    alpha = dkx@np.linalg.inv(self.K)
    beta = np.linalg.solve(np.transpose(self.params.gp_L), np.linalg.solve(self.params.gp_L, np.transpose(kk)))
