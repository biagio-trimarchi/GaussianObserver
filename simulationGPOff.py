...
class Simulation:
    def __init__(self):
        ...
        self.dist_fitter.set_hyperparams(np.ones((self.dim,))*0.5)
    ...
    def controller(self):
        ...
        self.dist_fun()
        self.hGP = self.dist_fitter.posterior_mean(self.p) - 4*self.dist_fitter.posterior_variance(self.p)
        gradGP = self.dist_fitter.posterior_dxmean(self.p) - 4*self.dist_fitter.posterior_dxvariance(self.p)
        hessGP = self.dist_fitter.posterior_ddxmean(self.p) - 4*self.dist_fitter.posterior_ddxvariance(self.p)
    ...
    def plotSafetySet(self):
        dom_x = np.arange(-4, 4.1, 0.4)
        dom_y = np.arange(-4, 4.1, 0.4)
        ...
        ax1.contour3D(xx, yy, zh, 25, cmap='coolwarm', edgecolor='none')
        plt.grid()

        fig2 = plt.figure()
        ax2 = plt.axes(projection='3d')
        ax2.contour3D(xx, yy, zv, 25, cmap='viridis', edgecolor='none')
        plt.grid()
    ...
    def run(self):
        ...
        for idx_x in range(dom_x.shape[0]):
            for idx_y in range(dom_y.shape[0]):
                self.dist_fitter.add_sample(np.array([dom_x[idx_x], dom_y[idx_y]]), self.distanceReadingsSmoothFromP(np.array([dom_x[idx_x], dom_y[idx_y]]).reshape((2, 1))))
