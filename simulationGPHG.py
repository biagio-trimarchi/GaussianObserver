...
class Simulation:
    def __init__(self):
        ...
        self.d_dist_fitter.set_hyperparams(np.ones((2*self.dim,))*0.5)
    ...
    def controller(self):
        ...
        hGP = self.zAct[0]/self.scale
        # hGP = self.h
        mu = 1
        LfHGP = self.d_dist_fitter.posterior_mean(self.xAct) - mu*self.d_dist_fitter.posterior_variance(self.xAct) 
        gradLfHGP = self.d_dist_fitter.posterior_dxmean(self.xAct) - mu*self.d_dist_fitter.posterior_dxvariance(self.xAct)
        ...
        Bopt = gradLfHGP.T @ self.Adyn @ self.xAct + \
                self.k1 * hGP + \
                self.alpha2 * LfHGP + self.alpha1 * self.zAct[1] /self.scale
    ...
    def run(self):
        ...
        # Initialize GP L_f h
        self.d_dist_fitter.add_sample(self.xAct, self.zAct[1]/self.scale)
        ...
        while t < self.T:
            ...
            for x in self.d_dist_fitter.params.gp_x.T:
                if np.linalg.norm(self.xAct - x.reshape((4,1))) < 0.1:
                    new = False
                    break
            if new:
                self.d_dist_fitter.add_sample(self.xAct, self.zAct[1]/self.scale)
            ...
            if self.tExp > 2.0 or np.linalg.norm(self.v) < 0.1:
                self.findNextGoal()
                self.tExp = 0.0
