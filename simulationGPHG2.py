...
class Simulation:
    def __init__(self):
        ...
        ### Parameters
        self.sm_a = 30.0   # Smooth min/max parameter
        self.safe_par = 0.1 
        ...
        self.d_dist_fitter.set_hyperparams(np.ones((2*self.dim,))*0.1)
        # Usefult quantities
    ...
    def distanceReadingsSmoothGoal(self):
        # Compute the distance of x from obstacle
        self.dG = []
        for obstacle in self.obstacles:
            self.dG.append(np.linalg.norm(self.pGoal-obstacle['Position'])**2 - (obstacle['Radius'] + self.safe_par)**2)

        self.dG = np.array(self.dG)
        self.hGG = self.shFun(self.dG)
    ...
    def controller(self):
        ...
        # hGP = self.zAct[0]
        hGP = self.h
        LfHGP = self.d_dist_fitter.posterior_mean(self.xAct) - 100*self.d_dist_fitter.posterior_variance(self.xAct) 
        gradLfHGP = self.d_dist_fitter.posterior_dxmean(self.xAct) - 100*self.d_dist_fitter.posterior_dxvariance(self.xAct)
    ...
    def findNextGoal(self):
        ...
        while not found:
            ...
            self.distanceReadingsSmoothGoal()
    ...
    def run(self):
        ...
        # Initialize GP L_f h
        self.d_dist_fitter.add_sample(self.xAct, self.zAct[1] + self.alpha1*self.h)
        ...
        while t < self.T:
            ...
            # if self.d_dist_fitter.posterior_variance(self.xAct) > 0.01:
            #     self.d_dist_fitter.add_sample(self.xAct, self.zAct[1] + self.alpha1*self.h)
            #     self.d_dist_fitter.train()
            #     print('train')
