...
#FIGURE_PATH = "/home/biagio/Scrivania/Report_Observers/figs/Simulation_1"
...
class Simulation:
    def __init__(self):
        ...
        self.obstacles.append({'Position': np.array([4.0, -2.0]).reshape((self.dim,1)), 'Radius': 0.5})
    ...
    def trajectory(self):
        self.pGoal = np.zeros((self.dim, 1))
        self.vGoal = np.zeros((self.dim, 1))
        
        w = 0.5
        self.pGoal[0] = self.t
        self.pGoal[1] = np.sin(w*np.pi*self.t)

        self.vGoal[0] = 1
        self.vGoal[1] = w*np.pi*np.cos(w*np.pi*self.t)
    ...
    def run(self):
        ...
        while self.t < self.T:
            ...
            for x in self.dist_fitter.params.gp_x.T:
                if np.linalg.norm(self.p - x.reshape((2,1))) < 0.2:
                    new = False
                    break
            if new:
                self.dist_fitter.add_sample(self.p, self.dist_fun())
