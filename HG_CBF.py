...
def main():
    ...
    goals.append(np.array([-3, 1]))
    goals.append(np.array([-3, 3]))
    goals.append(np.array([0, 0]))
    goals.append(np.array([3, -3]))
    goals.append(np.array([-3, 2]))
    ...
    print(fitterHD.get_hyperparams())
    ...
    while t < 5:
        ...
        # Update Goal
        if np.linalg.norm( x_act[0:2] - pGoal.T ) < 10**-2 or np.linalg.norm( x_act[0:2] - x[0:2, -2]) < 10**-6:
            idx_goal = idx_goal+1
    ...
    for idx_x in range(dom_x.shape[0]):
        for idx_y in range(dom_y.shape[0]):
            ...
            zz2[idx_x, idx_y] = fitterHD.posterior_mean(x)
    ...
    fake2Dline2 = mpl.lines.Line2D([0],[0], linestyle="none", c='g', marker = 'o')
