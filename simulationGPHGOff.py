...
class Simulation:
    def __init__(self):
        ...
        ### Parameters
        self.sm_a = 10.0   # Smooth min/max parameter
        self.safe_par = 0.1 
        ...
        self.d_dist_fitter.set_hyperparams(np.ones((2*self.dim,))*2)
        # Usefult quantities
    ...
    def distanceProduct(self):
        ...
        self.h = 1.0
        for obstacle in self.obstacles:
            self.h = self.h * (np.linalg.norm(self.p-obstacle['Position'])**2 - (obstacle['Radius'] + 0.1)**2)
        return self.h
    ...
    def controller(self):
        ...
        self.dist_fun()
        hGP = self.dist_fitter.posterior_mean(self.p)
        # hGP = self.zAct[0]
        # hGP = self.h
        LfHGP = self.d_dist_fitter.posterior_mean(self.xAct) - 10*self.d_dist_fitter.posterior_variance(self.xAct) 
        gradLfHGP = self.d_dist_fitter.posterior_dxmean(self.xAct) - 10*self.d_dist_fitter.posterior_dxvariance(self.xAct)

        self.LfhGP = np.append(self.LfhGP, self.d_dist_fitter.posterior_mean(self.xAct))
        

        # Quantities for plots
        self.psi0 = np.append(self.psi0, hGP)
        self.psi1 = np.append(self.psi1, LfHGP + self.alpha1*hGP)

        # Setup QP 
        Aopt = - gradLfHGP.T @ self.Bdyn
        Bopt = gradLfHGP.T @ self.Adyn @ self.xAct + \
                self.k1 * hGP + \
                self.alpha2 * LfHGP + self.alpha1 * self.zAct[1]
    ...
    def run(self):
        ...
        while t < self.T:
            ...
            print(self.d_dist_fitter.posterior_variance(self.xAct))
            if abs(self.d_dist_fitter.posterior_mean(self.xAct) - self.zAct[1]) > 0.4:
                self.d_dist_fitter.add_sample(self.xAct, self.zAct[1])
                self.d_dist_fitter.train()
                print('train')
                print(self.zAct[1])
                print(self.d_dist_fitter.posterior_mean(self.xAct))
            ...
            # if self.dist_fitter.posterior_variance(self.p) > 0.005:
            #     self.dist_fitter.add_sample(self.p, self.dist_fun())
            #     self.dist_fitter.train()
